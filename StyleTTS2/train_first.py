...
from wandb_utils import log_audio
from utils import *
from losses import *
...
def main(config_path):
    ...
    # torch 2.6: broadcasting mt19937 RNG state across ranks corrupts it
    # ("Invalid mt19937 state"), so disable RNG sync and keep shuffling
    # aligned across ranks with the seedable sampler instead
    dl_config = DataLoaderConfiguration(
        split_batches=True, use_seedable_sampler=True, data_seed=1234
    )
    ...
    if accelerator.is_main_process:
        ...
        init_wandb(config, name=osp.basename(log_dir) + "-stage1")
    ...
    # ── LR schedule: linear warmup, then halve on val-mel plateau ─────────────
    # config lr is the PEAK; the OneCycle schedulers from build_optimizer are
    # flat and never stepped, so LR is managed here via param_groups directly.
    grad_clip = float(config["optimizer_params"].get("grad_clip", 10.0))
    warmup_steps = int(config["optimizer_params"].get("warmup_steps", 300))
    lr_decay_factor = float(config["optimizer_params"].get("lr_decay_factor", 0.5))
    lr_patience = int(config["optimizer_params"].get("lr_patience", 3))
    min_lr_factor = float(config["optimizer_params"].get("min_lr_factor", 0.0625))
    base_lrs = {
        k: [g["lr"] for g in opt.param_groups] for k, opt in optimizer.optimizers.items()
    }

    def apply_lr_factor(factor):
        for k, opt in optimizer.optimizers.items():
            for g, base in zip(opt.param_groups, base_lrs[k]):
                g["lr"] = base * factor

    plateau_factor = 1.0

    es_params = config.get("early_stopping", {})
    es_patience = int(es_params.get("patience", 10))
    es_min_delta = float(es_params.get("min_delta", 0.001))
    epochs_since_best = 0
    ...
    if accelerator.is_main_process:
        ...
        if _baseline_voicepack is not None:
            ...
            for i, (text, audio) in enumerate(_baseline_audio):
                ...
                log_audio(f"baseline/test_{i + 1:02d}", audio, 0, sr, caption=text)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            apply_lr_factor(
                min(1.0, (iters + 1) / max(1, warmup_steps)) * plateau_factor
            )
            waves = batch[0]
            ...
            # clip generator grads — one bad batch NaN'd epoch 8 at lr 1.4e-4.
            # optimizer_params.grad_clip <= 0 disables (v1-replica mode).
            if grad_clip > 0:
                trained_keys = ["text_encoder", "style_encoder", "decoder"]
                if epoch >= TMA_epoch:
                    trained_keys += ["text_aligner", "pitch_extractor"]
                for key in trained_keys:
                    accelerator.clip_grad_norm_(
                        model[key].parameters(), max_norm=grad_clip
                    )
        ...
        # plateau LR decay + early stopping — same values on all ranks since
        # loss_test is built from accelerator.gather, so no broadcast needed
        avg_val = loss_test / iters_test
        if avg_val < best_loss - es_min_delta:
            best_loss = avg_val
            epochs_since_best = 0
            # first_stage.pth tracks the BEST val checkpoint (what Stage 2 loads)
            if accelerator.is_main_process:
                torch.save(
                    {
                    ...
                    osp.join(log_dir, config.get("first_stage_path", "first_stage.pth")),
                )
        else:
            epochs_since_best += 1
            if (
                epochs_since_best > 0
                and epochs_since_best % lr_patience == 0
                and plateau_factor > min_lr_factor
            ):
                plateau_factor = max(plateau_factor * lr_decay_factor, min_lr_factor)
                if accelerator.is_main_process:
                    log_print(
                        "Plateau: val mel stuck for %d epochs, LR factor -> %.4f"
                        % (epochs_since_best, plateau_factor),
                        logger,
                    )
        ...
        if accelerator.is_main_process:
            ...
            writer.add_scalar(
                "train/lr",
                optimizer.optimizers["decoder"].param_groups[0]["lr"],
                epoch + 1,
            )
            ...
            if _vp is not None and _test_tokens:
                ...
                for i, (text, audio) in enumerate(_epoch_audio):
                    ...
                    log_audio(
                        f"inference/test_{i + 1:02d}", audio, epoch + 1, sr, caption=text
                    )
            _ = [model[key].train() for key in model]
            # ───────────────────────────────────────────────────────────────────
        ...
        if epochs_since_best >= es_patience:
            if accelerator.is_main_process:
                log_print(
                    "Early stopping: no val mel improvement > %.4f for %d epochs "
                    "(best %.4f)" % (es_min_delta, epochs_since_best, best_loss),
                    logger,
                )
            break

    if accelerator.is_main_process:
        # first_stage.pth already holds the best-val checkpoint; only write the
        # final state here if no epoch ever improved on the pretrained init
