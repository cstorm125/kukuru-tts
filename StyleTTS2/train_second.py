...
def main(config_path):
    ...
    init_wandb(config, name=osp.basename(log_dir) + "-stage2")
    ...
    if load_pretrained:
        ...
        # Initialize predictor_encoder from trained style_encoder ONLY when
        # absent from the checkpoint (stage-1-style ckpt). When resuming a
        # stage-2 run the checkpoint carries a trained predictor_encoder —
        # deepcopying here would silently reset it.
        _ckpt_keys = torch.load(
            config["pretrained_model"], map_location="cpu"
        )["net"].keys()
        if "predictor_encoder" not in _ckpt_keys:
            model.predictor_encoder = copy.deepcopy(model.style_encoder)
