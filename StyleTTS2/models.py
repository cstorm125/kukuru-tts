...
def load_checkpoint(model, optimizer, path, load_only_params=True, ignore_modules=[]):
    ...
    for key in model:
        if key in params and key not in ignore_modules:
            sd = params[key]
            # align 'module.' prefixes between checkpoint (may be saved from a
            # DataParallel/DDP wrapper) and target: strict=False would
            # otherwise silently load NOTHING on mismatch
            model_keys = set(model[key].state_dict().keys())
            ckpt_prefixed = all(k.startswith("module.") for k in sd)
            target_prefixed = any(k.startswith("module.") for k in model_keys)
            if ckpt_prefixed and not target_prefixed:
                sd = {k[len("module."):]: v for k, v in sd.items()}
            elif target_prefixed and not ckpt_prefixed:
                sd = {"module." + k: v for k, v in sd.items()}
            missing, unexpected = model[key].load_state_dict(sd, strict=False)
            loaded = len(sd) - len(unexpected)
            print(f"{key} loaded ({loaded}/{len(model_keys)} tensors)")
            if loaded == 0 and len(model_keys) > 0:
                raise RuntimeError(
                    f"load_checkpoint: 0 tensors loaded for '{key}' from {path}"
                )
